...
## Grain density and four-scene sequence

The next review requested substantially more grains throughout the opening four scenes and removal of the separate project preview. The shared instanced field now contains 16,000 desktop grains (previously 3,900), with 2,800 on mobile (previously 1,100). Wider layered strands create a broad curved stream, and an irregular radius creates the fuller Vision wreath. Grain geometry, rotation, velocity response, native scrolling and the existing Work field remain unchanged. A peripheral hero layer and additional optical foreground grains add depth; only 72 field instances draw during the quiet initial hero, with the full field enabled when gathering starts. Existing DPR limits and reduced-motion behavior remain.

The “Made to matter” overlay is removed. The spatial composition now holds at the completed Vision phase while the existing compositor transitions directly into Work. No section is inserted, and no Work content, imagery, layout, camera path or links are changed. Logo and all three original identities remain intact.

Validation: production export/typecheck and all 15 tests pass. Full viewport screenshots compared at 1440×984, with additional checks at 1920×1080, 390×844 and 375×667. All sixteen opening frames have no horizontal overflow or page/shader errors. Separate desktop/mobile handoff checks confirm the preview overlay is absent, Vision remains visible through its endpoint, and the next section reaches KAO MING correctly. The higher desktop instance count increases GPU work; mobile remains substantially lighter, and physical-device GPU performance has not been measured. This remains a review branch, not a production deployment.

## Vision composition and reading pace

Scene four now uses a narrower, irregular wreath with finer grains and a strand entering from the left. The existing glass planes rotate into thin upper/left surfaces, a tall upper-right surface and a diagonal lower-right surface. Portrait fragments fade out as the ingredients converge; no screenshot preview appears inside the completed Vision frame. Glass reflections sharpen during this transition, internal triangulation edges are suppressed, and restrained grain highlights give the finer particles some light separation. The original people and Work assets remain unchanged.

Opening chapter units increase from 2.5/2.5 to 3.4/3.4 on desktop and from 1.65/1.9 to 2.1/2.4 on mobile. This adds approximately 44%/37% respectively to the travel before Work's incoming transition. The flow statement has a wider fully readable interval, ingredients have longer physical travel, and Vision stays visible until Work takes over. Native input, damping, the camera path, Work duration and all subsequent chapter durations are retained.

Validation: export/typecheck and 15 tests pass. Browser tests confirmed flow and ingredients remain at full opacity across 400 additional native scroll pixels on desktop and 220 on mobile. Sixteen opening frames fit without overflow; the desktop/mobile Vision-to-Work handoff passes. Full browser regression passes for five public project links, original portraits, mobile captions, Escape/focus, native wheel scrolling, Simple view restoration and OS reduced motion. No page/shader errors. Review-only branch; production unchanged.

## Final approval and ingredients reading hold

On 2026-09-12 the user approved publishing this refinement after slowing the ingredients scene. The shared story timeline now holds the complete phase-0.52 ingredients composition for an additional 1.05 viewport heights of native scrolling. Extra distance is added only to the identity chapter and then subtracted by the shared timeline, preserving earlier/later motion speed. The new hold is covered by a continuity/pacing regression test and verified through desktop/mobile browser scrolling. All 16 tests, export and full browser regression pass. See RELEASE.md for the approved publication record.
