  ...
  return <div className="people-depth">
    ...
    <div className="people-information">
      <div className="people-select interactive" role="group" aria-label="Meet the founders">
        {founders.map((founder, i) => <button key={founder.name} aria-pressed={i === active} onPointerEnter={e => {if(e.pointerType === 'mouse')setActive(i);}} onFocus={() => setActive(i)} onClick={() => setActive(i)} aria-controls="person-description"><span>{founder.name}</span><small>{founder.role}</small><i aria-hidden="true">↗</i></button>)}
      </div>
